...
import { getServiceById, updateService, updateServiceSchema } from '@portal/services';
...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const service = await getServiceById(user, params.id);
    if (!service) {
    ...
    return NextResponse.json({ error: 'Unable to fetch service' }, { status });
  }
}
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const user = await requireCurrentUser();
    const service = await updateService(user, params.id, parsed.data);
    return NextResponse.json({ service });
  } catch (error) {
